...
public class GameManagement : MonoBehaviour
{
    ...
    private List<GameObject> PlayerAvailableCards = new List<GameObject>();
    ...
    void Update()
    {
        if (CardToMove && IsMoving && !CardToMove.transform.position.Equals(TargetDeckPos))
        {
            ...
            CardToMove.transform.position = Vector3.MoveTowards(CurrPos, TargetDeckPos, MoveSpeed);
   ...
   private IEnumerator GameLoop()
    {
        ...
        if(!TimeSelect.activeSelf)
        {
            TimeSelect.SetActive(true);
        }
        while (TimeSelect.activeSelf)
        {
            yield return null;
        }
    ...
    private IEnumerator PlayCard(GameObject CardToPlay, List<GameObject> CardList, List<GameObject> AvailableCardList)
    {
        ...
        else if (CardToPlay.GetComponent<Card>().CardType.Equals("Gun"))
        {
            ...
            CardToMove = CardToPlay;
            MoveCards(CardToMove, GameDeck, GameCards);
            while (IsMoving)
            {
                yield return null;
            }
    ...
    private void DisplayResults()
    {
        ...
        if (PlayerCards.Count < AICards.Count)
        {
            Winner.text = ("You Win!");
        }
        else if (PlayerCards.Count > AICards.Count)
        {
        ...
        PLCnt.text = "Player 1: " + PlayerCards.Count;
        AICnt.text = "Player 2: " + AICards.Count;
    }
    ...
    public void Set30()
    {
        GameTimerInSeconds = 30;
        TimeSelect.SetActive(false);
    }

    public void Set60()
    {
        GameTimerInSeconds = 60;
        TimeSelect.SetActive(false);
    }

    public void Set90()
    {
        GameTimerInSeconds = 90;
        TimeSelect.SetActive(false);
    }

    public void Set120()
    {
        GameTimerInSeconds = 120;
        TimeSelect.SetActive(false);
    }
}
